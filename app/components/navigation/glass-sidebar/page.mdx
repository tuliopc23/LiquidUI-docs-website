...
function AppSidebar() {
  const { isOpen, activeItem, toggleSidebar, setActiveItem } = useSidebarStore()
...
*Glass Sidebar provides a flexible, accessible side navigation solution that adapts to different screen sizes while maintaining the elegant Liquid Glass aesthetic.*
